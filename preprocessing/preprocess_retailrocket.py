...
DATA_PATH_PROCESSED = '../data/retailrocket/single/'
DATA_FILE = 'events'
...
MIN_ITEM_SUPPORT = 5
MAX_SESSION_LENGTH = 999999  # deactivated
# min date config
MIN_DATE = '2014-04-01'
...
def preprocess_org_min_date(path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED,
                            min_item_support=MIN_ITEM_SUPPORT, min_date=MIN_DATE,
...
def preprocess_days_test(path=DATA_PATH, file=DATA_FILE,
                         path_proc=DATA_PATH_PROCESSED,
...
def preprocess_days_test_min_date(path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED,
                                  min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH,
                                  max_session_length=MAX_SESSION_LENGTH, days_test=DAYS_TEST, min_date=MIN_DATE):
...
def preprocess_slices(path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED, min_item_support=MIN_ITEM_SUPPORT,
                      min_session_length=MIN_SESSION_LENGTH, max_session_length=MAX_SESSION_LENGTH, num_slices=NUM_SLICES,
                      days_offset=DAYS_OFFSET, days_shift=DAYS_SHIFT, days_train=DAYS_TRAIN, days_test=DAYS_TEST):
...
def filter_data(data, min_item_support, min_session_length, max_session_length):
    ...
    # output
    data_start = datetime.fromtimestamp(int(data['Time'].min()), timezone.utc)
    data_end = datetime.fromtimestamp(int(data['Time'].max()), timezone.utc)
